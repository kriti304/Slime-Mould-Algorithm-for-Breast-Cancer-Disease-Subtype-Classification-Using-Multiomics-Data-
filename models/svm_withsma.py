...
sma_fs = SMAFeatureSelection(n_agents=50, max_iterations=50)
best_solution, best_fitness = sma_fs.run()

selected_features_svm_sma_idx = np.where(best_solution == 1)[0]
selected_feature_names = [X.columns[i] for i in selected_features_svm_sma_idx]
...
joblib.dump(selected_features_svm_sma_idx, "selected_features_svm_sma.pkl")
print("✅ Selected feature indices saved as selected_features_svm_sma.pkl")

"""SVM CLASSIFICATION FOR ML MODEL"""
...
# --- Use SMA-selected features ---
X_selected = X_resampled[:, selected_features_svm_sma_idx]

# --- Train SVM Model ---
svm_model = SVC(kernel='rbf', C=1.5, probability=True, random_state=42)
svm_model.fit(X_selected, y_resampled)

# --- Save model ---
model_filename = "svm_withSMA.pkl"
...
print(f"\n🎯 Accuracy of SVM Model (SMA Features): {accuracy:.4f}")
...
plt.title("SVM Confusion Matrix (SMA-Selected Features)")
...
plt.ylabel("Count")
plt.title("Predicted Alive vs Dead Counts (SMA-Selected Features - SVM)")
plt.grid(axis='y', linestyle='--', alpha=0.7)
plt.tight_layout()
plt.show()
...
fig.suptitle("SVC Evaluation with SMA-Selected Features", fontsize=16, fontweight='bold')
