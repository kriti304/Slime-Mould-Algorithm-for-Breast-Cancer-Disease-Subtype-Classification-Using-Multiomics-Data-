...
# Define number of features from resampled dataset
num_features = X_resampled.shape[1]

# Fitness function using SVM
...
print("\n🚀 Starting Genetic Algorithm Feature Selection (SVM)\n")
...
selected_features_svm_ga_idx = [i for i, bit in enumerate(best_individual) if bit == 1]
selected_feature_names = [f"Feature {i}" for i in selected_features_svm_ga_idx]

print("\n✅ Genetic Algorithm Feature Selection Completed!")
print(f"Selected Feature Indices: {selected_features_svm_ga_idx}")
print(f"Total Selected Features: {len(selected_features_svm_ga_idx)}")

joblib.dump(selected_features_svm_ga_idx, "selected_features_svm_ga.pkl")
print("✅ Selected feature indices saved as selected_features_svm_ga.pkl")

# --- Final Model Evaluation ---
X_selected = X_resampled[:, selected_features_svm_ga_idx]
X_train, X_test, y_train, y_test = train_test_split(
    X_selected, y_resampled, test_size=0.2, stratify=y_resampled, random_state=42
)
...
joblib.dump(X_selected, "X_train_selected_svm_ga.pkl")
joblib.dump(y_resampled, "y_train_selected_svm_ga.pkl")
...
plt.title("Genetic Algorithm (SVM): Accuracy vs. Generation")
plt.grid(True)
plt.tight_layout()
plt.show()

selected_feature_names = [X.columns[i] for i in selected_features_svm_ga_idx]
...
print(f"\n📊 Accuracy of SVM Model: {accuracy:.4f}")
...
plt.title("SVM Confusion Matrix (GA Features)")
...
plt.ylabel("Count")
plt.title("Predicted Alive vs Dead Counts (GA-Selected Features - SVM)")
plt.grid(axis='y', linestyle='--', alpha=0.7)
plt.tight_layout()
plt.show()
...
fig.suptitle("SVC Evaluation with GA-Selected Features", fontsize=16, fontweight='bold')
